    ...
    <div class="main">
        ...
        <div class="filter-container">
            ...
            <select id="filterCategory" class="filter-select">
                ...
                <option value="bb_balita">Berat Badan</option>
                <option value="tb_balita">Tinggi Badan</option>
                <option value="lila_balita">Lingkar Lengan</option>
                <option value="lika_balita">Lingkar Kepala</option>
            ...
            <table>
                ...
                <tbody id="dataBody">
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo "<tr>";
                        echo "<td>{$no}.</td>";
                        echo "<td data-category='no_register'>{$row['no_register']}</td>";
                        echo "<td data-category='id_balita'>{$row['id_balita']}</td>";
                        echo "<td data-category='na_balita'>{$row['na_balita']}</td>";
                        echo "<td data-category='bb_balita'>{$row['bb_balita']}</td>";
                        echo "<td data-category='tb_balita'>{$row['tb_balita']}</td>";
                        echo "<td data-category='lila_balita'>{$row['lila_balita']}</td>";
                        echo "<td data-category='lika_balita'>{$row['lika_balita']}</td>";
                        echo "<td data-category='tanggal_pemeriksaan'>{$row['tanggal_pemeriksaan']}</td>";
                        echo "<td>
                            <div class='action-buttons'>
                                <a href='edit_pemeriksaan.php?id={$row['no_register']}' class='action-btn edit' title='Edit'>
                                    <i class='fas fa-edit'></i>
                                </a>
                                <a href='#' onclick=\"confirmDelete('{$row['no_register']}', '{$row['na_balita']}')\" class='action-btn delete' title='Hapus'>
...
function downloadPDF() {
    ...
    try {
        ...
        header.innerHTML = `
            <h2 style="color: #8e2de2; margin: 0; font-size: 18px;">Laporan Pemeriksaan Balita</h2>
        ...
        const opt = {
            ...
            filename: 'laporan_pemeriksaan_balita.pdf',
...
function confirmDelete(noRegister, namaBalita) {
    Swal.fire({
        icon: 'warning',
        title: 'Hapus Data Pemeriksaan?',
        html: `Apakah Anda yakin ingin menghapus data pemeriksaan balita <strong>${namaBalita}</strong> (No Register: ${noRegister})?<br><br><small style="color: #dc3545;">Tindakan ini tidak dapat dibatalkan!</small>`,
        ...
        if (result.isConfirmed) {
            ...
            window.location.href = `hapus_pemeriksaan.php?id=${noRegister}`;
        }
    });
}
